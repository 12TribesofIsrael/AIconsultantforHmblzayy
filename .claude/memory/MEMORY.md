...
- [Node TLS workaround](reference_node_tls_workaround.md) — Tracker-script invocation differs PER MACHINE: Deskt(Node22+) needs `$env:NODE_OPTIONS='--use-system-ca'`; Owner(Node20) rejects that flag — run `node scripts/x.js` directly (NODE_EXTRA_CA_CERTS set; npm wrappers fail with "Could not determine Node.js install directory")
